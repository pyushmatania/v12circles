...
// --- CONFIG ---
const DATA_PATH = path.join(__dirname, '../src/data/realCommunityData.ts');
...
function loadCommunityData() {
  ...
  // Remove TS export and parse as JSON
  const jsonStr = raw.replace(/export const realCommunityData\s*=\s*/, '').replace(/;?\s*$/, '');
  return eval('(' + jsonStr + ')');
}
